...
- Resumes (not pitch pages): mirror the JD's literal phrasing in skills and bullets, no synonyms ("customer relationship management" if that's what they wrote, not "CRM"). ATS matching is string-literal; pitch pages are read by humans and don't need this.
- Banned sentence pattern (Sujin, 2026-08-04): an observation followed by a self-referential verdict tail, e.g. "That is the only way I have ever worked." / "That is a filter most companies are too polite to write down." / "That's the room I work best in." Reads as AI. State the fact and stop, or fold the stance into the same sentence.
- The competitor-swap test: any "why I like you" beat or "why us" answer must fail when a rival's name is swapped in. If the line still works for a competitor, rewrite it around something only this company did.
